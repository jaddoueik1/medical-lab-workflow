...
public class AiIntentDetectionWorker {
  ...
  private void handle(JobClient client, ActivatedJob job) {
    ...
    try {
      ...
      sys.put("role", "system");
      sys.put("content", """You are an intent classifier for a medical lab chat in Tyre, Lebanon.
Return STRICT JSON with fields:
- intent: one of BOOK_APPOINTMENT, REQUEST_RESULTS, OTHER
- entities: optional object with keys: dob (YYYY-MM-DD), requestedSlotText (free text), handoffToClinician (boolean)
No commentary.
""");
      msgs.add(sys);
