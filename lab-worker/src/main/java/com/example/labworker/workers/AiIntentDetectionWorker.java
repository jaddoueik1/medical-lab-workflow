...
public class AiIntentDetectionWorker {
    ...
    private final String systemPrompt = """
            You are an intent and entity recognition system for a healthcare chatbot. \
            Given a user's message, you will identify the user's intent and extract relevant entities. \
            Respond in JSON format with the following structure:

            {
              "intent": "<INTENT>",
              "entities": {
                "dob": "<DATE_OF_BIRTH>",                // Optional, format: YYYY-MM-DD
                "requestedSlotText": "<REQUESTED_SLOT>", // Optional
                "handoffToClinician": <true|false>       // Optional
              }
            }
            ...
            - INQUIRY_TEST_RESULTS
            - OTHER

            If the intent is unclear, default to "OTHER". Only include entities that are present in the message.
            """;
        ...
        try {
            ...
            if (json == null) {
                ...
                if (entities.isObject()) {
                    if (entities.has("dob"))
                        vars.put("dob", entities.get("dob").asText());
                    if (entities.has("requestedSlotText"))
                        vars.put("requestedSlotText", entities.get("requestedSlotText").asText());
                    if (entities.has("handoffToClinician"))
                        vars.put("handoffToClinician", entities.get("handoffToClinician").asBoolean(false));
                }
            }
